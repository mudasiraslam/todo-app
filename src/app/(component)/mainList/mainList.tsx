...
const AddTodo: React.FC = () => {
  ...
  return (
    <div
      className={`bg-custom-radical bg-dotted-size ${selectedTheme?.bgClass || `${color.defaultVibrantBg}`
        }`}
    >
      <div
        className={`${selectedTheme?.textClass || ''
          } bg-dotted-size backdrop-blur-backdropBlur bg-blend-overlay py-6 pl-6`}
      >
      ...
      <div className={`flex items-center justify-center min-h-screen`}>
        <div>
          <form onSubmit={handleSubmit}>
            ...
            <div
              className={`flex flex-wrap justify-center gap-3 max-w-xl mx-auto mt-8`}
            >
              {themes.map((theme, index) => (
                <button
                  ...
                  className={`px-6 py-1 rounded-3xl border-4 ${selectedTheme?.value === theme.value
                      ? 'border-4'
                      : ''
